...
for(const token of ["spawnSync(file,args",'shell:false','frontendTests=[','test-phase13-closeout.mjs','validate-phase14-closeout.mjs','test-phase15-design-system.mjs',"process.platform==='win32'?'npm.cmd':'npm'",'--backend','python,[\'-m\',\'pytest\']','database hash changed','Phase15FunctionalQualification.v1','phase15_9_qualification.json'])assert.ok(runner.includes(token),token);
