...
export default function TeachingPanel({ selection, analysis, provenance, onClose, onCorrectionMutation, lastTeachingReceipt, onTeachingReceipt }) {
  ...
  return <section className="teaching-panel teaching-workflow teaching-surface" data-testid="teaching-panel" role="dialog" aria-modal="true" aria-labelledby="teaching-surface-title">
    <div className="teaching-panel-header teaching-panel-toolbar"><div><span className="teaching-eyebrow">Reader Teaching</span><strong id="teaching-surface-title">Teaching review</strong><span>{showExisting ? 'Reviewed occurrence' : 'Inspect, preview, then review'}</span></div><div className="teaching-toolbar-actions"><button type="button" className="secondary" onClick={() => setAdvancedOpen(value => !value)}>{advancedOpen ? 'Hide advanced tools' : 'Advanced tools'}</button><button type="button" className="secondary" onClick={onClose} aria-label="Close Teaching review">Close</button></div></div>
    ...
    {showPreview && <><ol className="teaching-stepper"><li className="active"><span>1</span>Preview</li><li><span>2</span>Teaching review</li></ol><div className="teaching-stage-card">{editMode && hasExisting && <div className="teaching-edit-warning"><strong>Editing an existing reviewed occurrence</strong><span>Saving changed Teaching evidence will supersede the active record and retain its history.</span></div>}<div className="teaching-selection-summary"><strong>{selection.surface}</strong><span>Offsets {selection.start}-{selection.end} · spans {selection.coveredSpanIndexes.join(', ')}</span></div><div className="teaching-partition"><span>Current sentence</span><code>{partition(analysis.words)}</code></div>
      ...
      <div className="teaching-primary-actions"><button type="button" disabled={busy || (intent === 'keep-current' && !observed) || (intent === 'split' && !splitOffsets.length)} onClick={runPreview}>Preview result</button>{editMode && hasExisting && <button type="button" className="secondary" onClick={() => setEditMode(false)}>Cancel editing</button>}</div>{status.text && <div className={`status-message ${status.type}`} role="status" aria-live="polite">{status.text}</div>}
