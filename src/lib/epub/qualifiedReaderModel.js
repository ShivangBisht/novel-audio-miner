...
export function qualifyShadowRuntime(runtime){
 if(!runtime){
  return{
   valid:false,
   reason:'shadow-runtime-missing'
  };
 }
...
export async function buildQualifiedReaderModel({runtime,zip}={}){const gate=qualifyShadowRuntime(runtime);if(!gate.valid){
